...
const catController = createRouter((req, res, next) => {
    let newReq = req as any as {
        locals: any
    };

    newReq.locals = 'good cats!';

    next();
    return newReq;
});

catController.get('', (req, res) => {
    res.send({localsValue: req.locals});
});

route.setSubRouter('/onecat', catController);

const app = createApp();
app.setRouter('/cats', route);
app.listen(8080, () => {
